...
if (isEdit && tournament.results?.excludedPlayers) {
  ...
  const activeMembers = (teams[slotNumber] || []).filter(
      ...
      if (isTeamMode()) {
        ...
        resultsData = {
          first: winners.first ? {
            ...
            prizePerMember:selectedTournament?.prize1 /firstTeam.filter(p => !excludedPlayers.includes(p.odeuId)).length
          } : null,
          second: winners.second ? {
            ...
            prizePerMember:selectedTournament?.prize2 /secondTeam.filter(p => !excludedPlayers.includes(p.odeuId)).length
          } : null,
          third: winners.third ? {
            ...
            prizePerMember:selectedTournament?.prize3 /thirdTeam.filter(p => !excludedPlayers.includes(p.odeuId)).length
          } : null,
